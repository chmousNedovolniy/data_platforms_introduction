...
def main():
    spark = (
        SparkSession.builder.appName("task5-prefect-spark")
        .enableHiveSupport()
        .getOrCreate()
    )

    input_path = os.environ.get("TASK5_INPUT_PATH")
    output_table = os.environ.get("TASK5_OUTPUT_TABLE")
    if not input_path or not output_table:
        raise SystemExit("TASK5_INPUT_PATH or TASK5_OUTPUT_TABLE is not set")
