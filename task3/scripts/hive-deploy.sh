...
ensure_packages() {
  ...
  remote_sudo "${host}" "DEBIAN_FRONTEND=noninteractive apt-get update -y && DEBIAN_FRONTEND=noninteractive apt-get install -y curl tar libderby-java libderbyclient-java"
}
...
export HIVE_AUX_JARS_PATH=/usr/share/java/derbyclient.jar
...
prepare_hdfs() {
  ...
  local hdfs_cmd
  hdfs_cmd="export JAVA_HOME=${JAVA_HOME}; export HADOOP_HOME=${HADOOP_HOME}; export HADOOP_CONF_DIR=${HADOOP_HOME}/etc/hadoop; \
...
${HADOOP_HOME}/bin/hdfs dfs -fs ${HDFS_NAMENODE_URI} -chmod 777 ${HDFS_WAREHOUSE};"
  remote_sudo "${HIVESERVER2_HOST}" "su - ${HIVE_USER} -c '${hdfs_cmd}'"
}
